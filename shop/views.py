...
def home(request):
    ...
    recent_posts = Post.objects.filter(status=True).order_by('-created_at')[:4]
    
    context = {
        'recent_posts': recent_posts,
         'products' : products,
        # ... your other home data (products, etc)
    }
...
def shop(request, category_slug=None):
    categories = Category.objects.filter(parent__isnull=True)  # only parent categories
    ...
    if category_slug:
        ...
        # If it's a parent category → show all its subcategory products
